...
def main():
    print("Building MyChatBot application for macOS...")
    ...
    cmd = [
        ...
        "--clean",
        "--noconfirm",
        "--onedir",  # macOS build in onedir mode
        f"--name={app_name}",
        f"--icon={icon}" if icon else "",
        f"--distpath={dist_path}",
        f"--workpath={work_path}",
        f"--osx-bundle-identifier=com.mychatbot.app"
    ]

    # Add target arch universal2 if on arm64 mac
    ...
    if result.returncode == 0:
        ...
        print(f"Executable bundle can be found in: {os.path.abspath(dist_path)}")

        print("\nFor macOS:")
        print("1. You may need to codesign the application bundle:")
        print(f"   codesign --deep --force --sign - {dist_path}/{app_name}.app")
        print("2. To create a DMG:")
        print(f"   hdiutil create -volname {app_name} -srcfolder {dist_path}/{app_name}.app -ov -format UDZO {dist_path}/{app_name}.dmg")
